...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/12/22 12:33:21 by haghouli          #+#    #+#             */
/*   Updated: 2023/01/10 05:43:27 by haghouli         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	key_handler(int key, t_data *data)
{
	...
	if (key == 53)
		exit(0);
	if (key == 13)
		return (move_vertical(data, &counter, &c, -1));
	else if (key == 1)
		return (move_vertical(data, &counter, &c, 1));
	else if (key == 0)
		return (move_horizontal(data, &counter, &c, -1));
	else if (key == 2)
		return (move_horizontal(data, &counter, &c, 1));
	return (1);
}
...
static int	check_errors(t_data *data)
{
	if (data->graph == NULL || is_map_error(data->graph)
		...
		|| check_elements(data->graph))
		return (0);
	if (check_stuck(data->graph, &((t_dim){ft_strlen(data->graph[0]),
				count_col(data->graph)}),
		&((t_dim){data->dim->y, data->dim->x})) != (count_collectors(data)
			+ 1))
